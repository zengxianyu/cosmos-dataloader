...
# Simplified logging - using print instead of complex logging system
...
from cosmos_datasets.lazy_config.file_io import PathManager
from cosmos_datasets.lazy_config.registry import _convert_target_to_string
...
class LazyConfig:
    ...
    def save_pkl(cfg, filename: str) -> str:
        ...
        try:
            with PathManager.open(filename, "wb") as f:
                pickle.dump(cfg, f)
            print(f"Config is saved using pickle at {filename}.")
        except Exception as e:
            print(f"Failed to save config to {filename}: {e}. Trying dill or cloudpickle instead")
            if dill_pickle:
                try:
                    with PathManager.open(filename, "wb") as f:
                        pickle.dump(dill_pickle.dumps(cfg, recurse=True), f)
                        print(f"Config is saved using dill at {filename}.")
                except Exception as e:
                    ...
                    if cloudpickle:
                        try:
                            ...
                            print(f"Config is saved using cloudpickle at {filename}.")
                        except Exception as e:
                            print(f"Failed to save config to {filename}: {e}.")
                    else:
                        print("cloudpickle is not available. Cannot save the config.")
                        raise e
    ...
    def save_yaml(cfg, filename: str) -> str:
                ...
                for key, value in config.items():
                    if isinstance(value, (DictConfig, ListConfig)):
                        try:
                            ...
                            print(f"Failed to add default argument values: {e}")
        ...
        print(f"Config is saved using omegaconf at {filename}.")
        return filename
