...
class OperationWatchdog:
    ...
    def start(self) -> None:
        ...
        if self._thread is None or not self._thread.is_alive():
            ...
            self._thread.start()
            print(f"[{self._name}] Watchdog monitoring thread started")
    ...
    def stop(self) -> None:
        ...
        if self._thread and self._thread.is_alive():
            ...
            if self._thread.is_alive():
                print(f"[{self._name}] Watchdog thread did not terminate within timeout", rank0_only=False)
            else:
                print(f"[{self._name}] Watchdog monitoring thread stopped", rank0_only=False)
    ...
    def print_stats(self) -> None:
        ...
        if not stats:
            print(f"[{self._name}] No operation statistics available", rank0_only=False)
            return

        print(f"[{self._name}] Operation Statistics:", rank0_only=False)
