...
class VideoParsing(Augmentor):
    ...
    def __call__(self, data_dict: dict) -> dict:
        ...
        if video_info["fps"] < self.min_fps:
            logger.warning(f"Video FPS {video_info['fps']} is less than min_fps {self.min_fps}")
            return None
        if video_info["fps"] > self.max_fps:
            logger.warning(f"Video FPS {video_info['fps']} is greater than max_fps {self.max_fps}")
            return None
