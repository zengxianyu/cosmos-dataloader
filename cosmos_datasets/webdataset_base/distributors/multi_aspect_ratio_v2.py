...
class ShardlistMultiAspectRatioInfinite(IterableDataset):
    ...
    def __init__(
        ...
        if self.verbose:
            print("ShardlistMultiAspectRatioInfinite init")
    ...
    def _allocate_workers_to_aspects(
        ...
        if self.verbose:
            print(f"Aspects: {aspect_keys}")
            print(f"Target ratio: {target_ratio}")
            print(f"Worker allocation: {aspect_worker_allocation}")
            print(f"Discrepancy: {aspect_worker_allocation / aspect_worker_allocation.sum() / target_ratio}")
    ...
    def _obtain_node_worker_url_mapping(
        ...
        if self.verbose:
            print(f"GID={global_worker_id}, aspect_key={aspect_key}, chunk_id={chunk_id}")
    ...
    def obtain_url_list(self):
        ...
        if self.verbose:
            print(f"PytorchShardList rank {rank} of {world_size}")
            print(f"PytorchShardList worker {worker_id} of {num_workers}")
        ...
        if self.verbose:
            print("List of urls (before shuffle)")
            print(urls[0:10])
        ...
        if self.verbose:
            print("List of urls (after shuffle)")
            print(urls[0:10])
